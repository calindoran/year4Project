...
public:
    ...
    /*! Create safe queue. */
    SafeQueue() = default;
    SafeQueue (SafeQueue&& sq) noexcept:
        m_mutex(), m_condition(), m_queue(std::move (sq.m_queue))
    {
    }
    ...
    */
    void set_max_num_items (size_t max_num_items)
    {
    ...
    */
    ...
    SafeQueue& operator= (SafeQueue && sq) noexcept
    {
        std::lock_guard<std::mutex> lock (m_mutex);
...
void swap (SafeQueue<T>& q1, SafeQueue<T>& q2)
{
    q1.swap (q2);
}
